//+------------------------------------------------------------------+
//|                                         MA_Channel_RSI_EA_v4.mq5 |
//|                              Simplified Trailing Stop Version    |
//|                                        Version 4.0 - CLEAN       |
//+------------------------------------------------------------------+
#property copyright "MA Channel RSI EA v4"
...
int OnInit()
{
    ...
    if(EnableLog)
    {
        ...
        {
            FileWrite(logFileHandle, "Time", "Type", "Price", "SL", "Pips", "Profit", "Balance");
        }
    }
...
void OnDeinit(const int reason)
{
    ...
    double finalBalance = AccountInfoDouble(ACCOUNT_BALANCE);
    Print("EA stopped | P/L: ", DoubleToString(finalBalance - startBalance, 2));
}
...
//| Log trade to file                                                 |
//+------------------------------------------------------------------+
void LogTrade(string type, double price, double sl, double pips, double profit)
{
    ...
    if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
    {
        ...
        if(HistoryDealSelect(dealTicket))
        {
            ...
            if(entry == DEAL_ENTRY_IN)
            {
                ...
                LogTrade(dir + "_OPEN", price, 0, 0, 0);
            }
            else if(entry == DEAL_ENTRY_OUT)
            {
                ...
                LogTrade("CLOSE", price, 0, pips, profit);
            }
        }
    }
}
